...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, error, helperText, children, className, containerClassName, id, ...props }, ref) => {
    ...
    return (
      <label className={cn('block', containerClassName)} htmlFor={selectId}>
        ...
        <select ref={ref} id={selectId} aria-invalid={Boolean(error)} className={cn('control-base w-full py-3', error && 'border-danger focus-visible:ring-danger', className)} {...props}>
          {children}
        </select>
